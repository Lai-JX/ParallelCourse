...
{  	
	/*
	VectorLength：ColsNo
	ScatterSize：每个MPI进程处理的行数
	ThreadDim：BLOCKSIZE
	MyRank：当前MPI进程的rank
	NumberofProcessors：MPI进程数
	 */
...
int main(int argc, char **argv)
{
	...
	ScatterSize = RowsNo / NumberOfProcessors;				// 每个进程处理的行数

	elements = (RowsNo * ColsNo2) / NumberOfProcessors;		// 每个进程处理结果的元素个数
    ...
    if(DeviceStatus == 0) {
		...
		for(int i = 0 ; i < ScatterSize ; i++) {
			...
			MyResultMatrix[i] += (MyMatrixA[IndexValue++] * VectorB[IndexCol]);	// 这里写的是矩阵向量乘法，实际应该是矩阵乘法
        }
	}
...
int CheckDevice(int MyRank)
{
	...
	struct cudaDeviceProp Properties;	// 包含当前 CUDA 设备的详细属性信息，如设备名称、计算能力等。

	cudaGetDeviceCount(&DeviceCount);	// 设备数量
	if(DeviceCount >= 1)
	{
